    ...
    context.coverage.cov.files.forEach((file) => {
        ...
        if (file.hits) {
            context.tests.forEach((test) => {
                ...
                if (!test.cov || !test.cov.files) {
                    return;
                }

                Object.keys(test.cov.files).forEach((covFilename) => {

                    if (file.filename !== covFilename) {
                        return;
                    }

                    test.cov.files[covFilename].forEach((line) => {
                        file.source[line].tests = file.source[line].tests || [];
                        file.source[line].tests.push(test.relativeTitle);
                    });
                });
            });
        }
    });
    console.log(JSON.stringify(context.coverage.cov))
