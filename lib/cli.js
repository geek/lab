...
exports.run = function () {
    ...
    if (settings.coverage) {
        ...
        const childCoverage = Path.join(__dirname, '/child_coverage.js');
        if (process.execArgv.indexOf(childCoverage) === -1) {
            process.execArgv.push('-r');
            process.execArgv.push(childCoverage);
            internals.startCoverageServer(settings);
        }
    }
...
internals.startCoverageServer = function (settings) {

    const file = Fs.createWriteStream(__dirname + '/../out');
    const server = Http.createServer((req, res) => {
        ...
        handleCoverage(req, res);
    });

    const handleSettings = function (req, res) {
        ...
        res.writeHead(200);
        res.end(JSON.stringif(settings));
    };

    const handleCoverage = function (req, res) {
        ...
        req.once('end', () => {
            ...
            try {
                ...
                file.write(data);
                Hoek.merge(global.__$$labCov, cov, false, true);
            }
            catch (err) {
                console.error(err);
            }
